...
class RlGame(gym.Env):
    def __init__(self, n,m,render=False):
        ...
        if self.Render:
            ...
            self.GRAPHICS = tools.load_graphics('D:/UVAfangtest/UAV-path-planning-main\Multi-UAVs path planning\path planning/assignment\source\image')

            #音频文件
            self.SOUND = tools.load_sound('D:/UVAfangtest/UAV-path-planning-main\Multi-UAVs path planning\path planning/assignment\source\music')
            #初始化时钟
        ...
        # else:
        #     self.dispaly=None
        #在每个时间步可以采取的动作是一个二维向量，其中每个分量的值介于 -1 和 1 之间。
        low = np.array([-1,-1])
        high=np.array([1,1])
    ...
    def start(self):
        ...
        self.set_enemy_image()#加载敌方单位的图像
        self.set_hero_image()#加载己方单位的图像
        self.set_obstacle_image()#加载障碍物的图像
        self.set_goal_image()#加载目标点的图像
    ...
    def set_hero_image(self):
        self.hero = self.__dict__ #创造一个领导无人机字典
        self.hero_group = pygame.sprite.Group()
        self.hero_image = self.GRAPHICS['fighter-blue']#存储图像
    ...
    def set_enemy_image(self):
        self.enemy = self.__dict__#跟随无人机字典
        self.enemy_group = pygame.sprite.Group()#存储图像
    ...
    def step(self,action):
        dis_1_obs = np.zeros((self.hero_num, 1))# 己方无人机与障碍物的距离
        dis_1_goal = np.zeros((self.hero_num+self.enemy_num, 1))# 己方和跟随着无人机与目标点的距离
        r=np.zeros((self.hero_num+self.enemy_num, 1))
        o_flag = 0# 障碍物碰撞标志
        o_flag1 = 0# 跟随者无人机障碍物碰撞标志
        #空气阻力系数
        F_k=0.08
        ...
        for i in range(self.hero_num+self.enemy_num):
            ...
            if i==0:#leader
                ...
                if self.hero['hero' + str(i)].posy >= C.ENEMY_AREA_HEIGHT:
                    ...
                    edge_r[i] = -1
                if 0 < dis_1_agent_0_to_1 < 50: #and dis_1_agent_0_to_2<50 and dis_1_agent_0_to_3<50 and dis_1_agent_0_to_4<50:
                    follow_r0=0
                    self.team_counter+=1
        ...
        #返回新的状态 hero_state、奖励 r、完成标志 done、胜利标志 win、团队计数器 team_counter 和无人机之间的距离 dis_1_agent_0_to_1
        return hero_state,r,done,self.hero['hero0'].win,self.team_counter #,dis_1_agent_0_to_1
    def render(self):
        #pygame.QUIT：处理窗口关闭事件，退出 Pygame 并终止程序。
        #pygame.MOUSEMOTION：处理鼠标移动事件，更新鼠标位置。
        #C.CREATE_ENEMY_EVENT：处理自定义事件，设置敌方单位创建标志。
