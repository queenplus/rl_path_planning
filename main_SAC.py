...
shoplistfile = 'D:/UVAfangtest/UAV-path-planning-main\Multi-UAVs path planning\path planning\MASAC_new1'  #保存文件数据所在文件的文件名
shoplistfile_test = 'D:/UVAfangtest/UAV-path-planning-main\Multi-UAVs path planning\path planning\MASAC_d_test2'  #保存文件数据所在文件的文件名
shoplistfile_test1 = 'D:/UVAfangtest/UAV-path-planning-main\Multi-UAVs path planning\path planning\MASAC_compare'  #保存文件数据所在文件的文件名
...
def run(env):
    if Switch==0:
        try:
            ...
            for k in range(TRAIN_NUM):
                ...
                for episode in range(EP_MAX):
                    ...
                    if episode % 20 == 0 and episode > 200:#保存神经网络参数
                        ...
                        torch.save(save_data, "D:/UVAfangtest/UAV-path-planning-main\Multi-UAVs path planning\path planning\Path_SAC_actor_L1.pth")
                        save_data = {'net': actors[1].action_net.state_dict(), 'opt': actors[1].optimizer.state_dict()}
                        torch.save(save_data, "D:/UVAfangtest/UAV-path-planning-main\Multi-UAVs path planning\path planning\Path_SAC_actor_F1.pth")
            ...
            print("Rtotlemax",all_ep_r_max)
            print("Rtotlemin", all_ep_r_min)
            print("all_ep_r_std",all_ep_r_std )
            #画图
            plt.margins(x=0)#设置 x 轴的边距为 0，确保图表的 x 轴紧贴数据。
            plt.plot(np.arange(len(all_ep_r_mean)), all_ep_r_mean, label='MASAC_Total reward', color='#e75840')
            ...
            plt.figure(2, figsize=(8, 4), dpi=150)
            plt.margins(x=0)
            plt.plot(np.arange(len(all_ep_L_mean)), all_ep_L_mean, label='MASAC_Leader reward', color='#4ada91')
            ...
            plt.figure(3, figsize=(8, 4), dpi=150)
            plt.margins(x=0)
            plt.plot(np.arange(len(all_ep_F_mean)), all_ep_F_mean, label='MASAC_Follower reward', color='#1ebdf2')
        ...
        print('SAC测试中...')
        aa = Actor()
        checkpoint_aa = torch.load("D:/UVAfangtest/UAV-path-planning-main\Multi-UAVs path planning\path planning\Path_SAC_actor_L1.pth")
        aa.action_net.load_state_dict(checkpoint_aa['net'])
        bb = Actor()
        checkpoint_bb = torch.load("D:/UVAfangtest/UAV-path-planning-main\Multi-UAVs path planning\path planning\Path_SAC_actor_F1.pth")
